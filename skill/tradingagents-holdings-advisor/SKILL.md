...
0. **Parse intent + archive context (Phase 0)**: Parse the current request and current holdings source first. After current codes are confirmed, call `GET /archives/context?codes=...&limit=5` when `ADVISOR_API_URL` and `ADVISOR_TOKEN` are configured. Use the response only for recent advice consistency, same-day reversal checks, and historical reduction detection. Do not call legacy persistence endpoints for history. The current backend integration for the skill is archive-only; see `references/persistence.md`.
...
- Do not treat a losing position as an automatic reduce/sell. A loss only raises risk priority; the final action must be evidence-gated as hold, conditional reduce, reduce, or conditional add from trend, relative strength, fund flow, sector position, news risk, and portfolio exposure.
- Do not recommend reduce/sell on a losing holding without naming the confirming evidence such as relative weakness, broken support, capital outflow, sector rollover, negative news, oversized exposure, or T+1/price-limit risk.
