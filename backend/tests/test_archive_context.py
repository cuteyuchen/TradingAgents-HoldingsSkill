...
def _upload_archive(client, headers, idx: int, holdings, suffix: str) -> int:
            ...
            "meta": json.dumps(
                {
                    ...
                    "title": f"Archive {suffix} {idx}",
                },
                ensure_ascii=False,
            )
        },
        files={
            "screenshot": (f"holdings-{suffix}-{idx}.png", b"\x89PNG\r\n\x1a\n", "image/png"),
            "holdings_json": (
                "holdings.json",
...
def test_archive_context_returns_recent_holdings_and_advice():
    ...
    for idx in range(6):
        ...
        archive_ids.append(_upload_archive(client, headers, idx, holdings, suffix))

    assert client.get("/api/v1/archives/context").status_code == 401

    response = client.get(f"/api/v1/archives/context?codes={CODE_A}&limit=5", headers=headers)
    assert response.status_code == 200, response.text
    payload = response.json()
    assert len(payload["archives"]) == 5
    assert "advice_md" not in payload["archives"][0]
    assert len(payload["timeline_by_code"][CODE_A]) == 5
    assert payload["latest_by_code"][CODE_A]["qty"] == 105
    assert payload["latest_by_code"][CODE_A]["archive_id"] == payload["archives"][0]["id"]
    assert payload["same_day_advice"][0]["archive_id"] == payload["archives"][0]["id"]
    assert "持有" in payload["same_day_advice"][0]["advice_excerpt"]

    response = client.get(
        f"/api/v1/archives/context?codes={CODE_B}&limit=5&include_advice=true",
        headers=headers,
    )
    assert response.status_code == 200, response.text
    filtered = response.json()
    assert len(filtered["archives"]) == 5
    assert set(filtered["timeline_by_code"]) == {CODE_B}
    assert filtered["latest_by_code"][CODE_B]["name"] == "测试标的B"
    assert "advice_md" in filtered["archives"][0]

    response = client.get(f"/api/v1/archives/context?codes={CODE_A}&limit=20", headers=headers)
    assert response.status_code == 200, response.text
    filtered = response.json()
    assert len(filtered["archives"]) == 6
    assert len(filtered["timeline_by_code"][CODE_A]) == 6
