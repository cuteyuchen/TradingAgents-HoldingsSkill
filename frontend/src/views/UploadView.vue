...
const stageLabels: Record<string, string> = {
  ...
  final_quote_refresh: '刷新最终行情', report_rendering: '生成结构化报告', completed: '分析完成', failed: '分析失败', cancelled: '已取消',
}
...
<template>
  <section class="page-stack">
    ...
    <n-alert v-else-if="snapshot && !selectedFile && !upload" type="success" :show-icon="false">
      已载入 <strong>{{ selectedPortfolio?.name }}</strong> 的最新确认快照 #{{ snapshot.id }}（{{ fmt(snapshot.snapshot_time) }}），可以直接在页面下方手动触发分析，也可以上传新截图覆盖今日持仓。
    </n-alert>

    <div class="upload-grid">
      <section class="panel-card upload-panel">
        <div class="section-title"><div><h2>1. 上传新持仓（可选）</h2><p>不上传新截图时，可直接分析最近一次已确认快照</p></div><FileImage :size="21" /></div>
      ...
      <section class="panel-card state-panel">
        ...
        <n-spin v-if="loadingLatestSnapshot" size="small" description="正在读取最新确认快照" />
