...
const candidateContext = ref<FuyaoSecurityContext | null>(null)
...
function briefCount(value: unknown): string {
  if (value === null || value === undefined || value === '') return unavailableText
  const parsed = Number(value)
  return Number.isFinite(parsed) ? String(Math.round(parsed)) : unavailableText
}
...
async function openCandidate(row: AnyRecord) {
  candidateDetail.value = row
  candidateContext.value = null
  candidateContextLoading.value = true
  if (row.code) candidateContext.value = await api.getFuyaoSecurityContext(String(row.code)).catch(() => null)
  candidateContextLoading.value = false
}

async function loadMarketBrief() {
  marketBrief.value = await api.getFuyaoMarketBrief().then((payload) => payload.brief).catch(() => null)
}
  ...
  <section class="workbench-page">
    ...
    <template v-if="detail">
      ...
      <template v-else>
        ...
        <SectionCard v-if="marketBrief" title="今日市场上下文" description="Fuyao 提供的市场、行业与情绪证据，仅用于解释今天发生了什么，不改变最终决策。">
          <div class="context-grid">
            <article><span>主要指数</span><div v-for="item in marketBrief.major_indices" :key="item.thscode || item.name" class="context-row"><strong>{{ item.name || item.thscode || '指数' }}</strong><b :class="sourcePercentClass(item.change_pct)">{{ sourcePercent(item.change_pct) }}</b></div><p v-if="!marketBrief.major_indices.length" class="muted">{{ unavailableText }}</p></article>
            <article><span>行业强弱</span><div class="context-pair"><div><small>领涨</small><strong>{{ marketBrief.industry.leaders?.[0]?.name || unavailableText }}</strong><b :class="sourcePercentClass(marketBrief.industry.leaders?.[0]?.change_pct)">{{ sourcePercent(marketBrief.industry.leaders?.[0]?.change_pct) }}</b></div><div><small>领跌</small><strong>{{ marketBrief.industry.laggards?.[0]?.name || unavailableText }}</strong><b :class="sourcePercentClass(marketBrief.industry.laggards?.[0]?.change_pct)">{{ sourcePercent(marketBrief.industry.laggards?.[0]?.change_pct) }}</b></div></div></article>
            <article><span>市场情绪</span><div class="sentiment-grid"><div><small>涨停</small><strong>{{ briefCount(marketBrief.sentiment.limit_up_count) }}</strong></div><div><small>跌停</small><strong>{{ briefCount(marketBrief.sentiment.limit_down_count) }}</strong></div><div><small>异动</small><strong>{{ briefCount(marketBrief.sentiment.abnormal_count) }}</strong></div><div><small>龙虎榜</small><strong>{{ briefCount(marketBrief.sentiment.dragon_tiger_count) }}</strong></div></div></article>
          </div>
        </SectionCard>
